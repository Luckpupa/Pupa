Shader "Custom/Toon/Water Tut"
{
    Properties
    {
		
		_DepthGradientShallow("Depth Gradient Shallow", Color) = (0.325, 0.807, 0.971, 0.725)//일반적인 물 색깔
		
		
		_DepthGradientDeep("Depth Gradient Deep", Color) = (0.086, 0.407, 1, 0.749)//깊은물에서의 색깔

		
		_DepthMaxDistance("Depth Maximum Distance", Float) = 1// 물깊이의 정도

		
		_FoamColor("Foam Color", Color) = (1,1,1,1)// / 흐르는 물색깔

		
		_SurfaceNoise("Surface Noise", 2D) = "white" {}//노이즈텍스쳐(물이동표시)

		
		_SurfaceNoiseScroll("Surface Noise Scroll Amount", Vector) = (0.03, 0.03, 0, 0)// 노이즈 움직이는 속도. Only the xy components are used.

		
		_SurfaceNoiseCutoff("Surface Noise Cutoff", Range(0, 1)) = 0.777// 노이즈 알파컷아웃 정도조절

		// Red and green channels of this texture are used to offset the
		// noise texture to create distortion in the waves.
		_SurfaceDistortion("Surface Distortion", 2D) = "white" {}	//RG채널 텍스처 이용
